...
특히 Gate Length가 감소하면 소자를 작게 만들고 높은 성능을 얻을 수 있지만, 지나치게 감소하면 Short Channel Effect의 영향이 증가할 수 있다.

---

## 3. Minimum Gate Length

GPDK090에서는 소자의 종류와 동작 전압에 따라 최소 Gate Length를 제한한다.

Gate Length가 지나치게 짧아지면 Drain Electric Field가 Source 측 Channel Potential에 영향을 주면서 다음과 같은 문제가 증가할 수 있다.
...
따라서 Minimum Gate Length Rule은 **Lithography를 통한 안정적인 Gate Pattern 형성과 MOSFET의 Channel 특성 확보**를 위한 중요한 규칙이다.

---

## 4. 고전압 Device와 Gate Length

2.5 V와 같이 높은 전압에서 동작하는 MOSFET에서는 Drain 부근에 더 큰 Electric Field가 형성될 수 있다.

높은 Electric Field에 의해 Carrier가 큰 운동 에너지를 얻으면 **Hot Carrier Injection(HCI)**이 발생할 수 있다.

고에너지 Carrier가 Gate Oxide 또는 Si-SiO₂ Interface에 주입되거나 Trap에 포획되면 장기적으로 다음과 같은 열화가 발생할 수 있다.
...
따라서 고전압 Device에서는 적절한 Gate Length와 Device Geometry를 통해 높은 Electric Field에 대한 신뢰성 Margin을 확보하는 것이 중요하다.

---

## 5. Minimum POLY Interconnect Width

POLY를 Gate 이외의 Interconnect로 사용할 경우에도 Minimum Width Rule이 적용된다.

POLY Interconnect가 지나치게 좁으면 Lithography 및 Etching 과정에서 Pattern 형성이 불안정해질 수 있으며 전기적인 저항도 증가한다.

POLY의 저항은 Sheet Resistance를 이용하면 다음과 같이 표현할 수 있다.
...
Minimum POLY Width Rule은 **POLY Pattern의 제조 안정성을 확보하고 지나친 저항 증가를 방지하기 위한 규칙**으로 이해할 수 있다.
...
GPDK090에서는 POLY의 사용 목적에 따라 다음과 같이 Spacing Rule이 구분될 수 있다.
...
를 통해 Mask Misalignment가 발생하더라도 Gate가 Active Area를 완전히 가로지를 수 있도록 한다.
...
따라서 Gate를 기준으로 Source/Drain 방향에 일정 크기 이상의 Active Area를 확보하여 **Gate와 Source/Drain의 상대적인 Geometry를 안정적으로 유지**한다.
...
POLY와 Active 사이의 간격이 지나치게 작으면 Overlay 및 Patterning Variation으로 인해 의도하지 않은 Gate-like Geometry가 형성될 가능성이 있다.
...
GPDK090에서는 Bent Gate 및 Bent POLY Resistor를 허용하지 않는 Rule이 존재한다.

Gate가 꺾이면 위치에 따라 유효 Channel Length가 달라질 수 있으며 Corner 부분에서 Electric Field와 Lithography 특성이 불균일해질 수 있다.

또한 POLY Resistor가 꺾이면 Corner의 실제 Current Path와 유효 Width 때문에 단순한 $L/W$ 관계만으로 저항값을 정확하게 예측하기 어려워진다.

$$
R = R_{sh}\frac{L}{W}
$$

따라서 직선 Geometry를 이용하여 소자 및 저항의 특성을 보다 균일하고 예측 가능하게 유지한다.
...
GPDK090에서는 전체 Chip에 대한 POLY Density Ratio의 최대값을 제한한다.
...
Minimum Width를 만족하더라도 전체 면적이 지나치게 작은 POLY Pattern은 Lithography 및 Etching 과정에서 안정적으로 형성되지 않거나 소실될 가능성이 있다.

$$
A_{\text{POLY}} \ge A_{\text{min}}
$$
...
특히 Gate Length Rule은 단순한 Patterning 제한을 넘어 MOSFET의 Channel Length를 직접 결정한다.
...
반면 Gate Extension과 POLY-to-Active Spacing은 서로 다른 Mask 사이의 Overlay Error에 대한 Process Margin이라는 의미가 강하다.

Bent POLY 및 Minimum Area Rule을 통해서는 단순히 Width와 Spacing만 만족한다고 모든 Geometry가 안정적으로 제조될 수 있는 것은 아니며, Corner와 작은 Pattern의 형태까지 고려해야 한다는 점을 확인하였다.

또한 POLY Interconnect와 POLY Resistor에서는

$$
R = R_{sh}\frac{L}{W}
$$

관계가 중요하기 때문에 동일한 POLY Layer라도 Gate, Interconnect, Resistor라는 용도에 따라 서로 다른 Design Rule이 필요한 이유를 이해할 수 있었다.
